...
import { PageGrant, type IPage, GroupType, getIdForRef } from '@growi/core';
...
describe('UserGroupService', () => {
  ...
  describe('updateGroup', () => {
    ...
    test('User should be included to parent group (2 groups ver)', async () => {
      ...
      const updatedUserGroup = await userGroupService.updateGroup(userGroup4?._id, userGroup4?.name, userGroup4?.description, groupId5, forceUpdateParents);
    ...
    test('User should be included to parent group (3 groups ver)', async () => {
      ...
      const forceUpdateParents = true;
      await userGroupService.updateGroup(userGroup8?._id, userGroup8?.name, userGroup8?.description, groupId7, forceUpdateParents);
    ...
    test('Should throw an error when trying to choose parent from descendant groups.', async () => {
      ...
      const result = userGroupService.updateGroup(userGroup9?._id, userGroup9?.name, userGroup9?.description, userGroup10?._id);
  ...
  describe('removeUserByUsername', () => {
    test('User should be deleted from child groups when the user excluded from the parent group', async () => {
      ...
      await userGroupService.removeUserByUsername(userGroup11?._id, 'ug_test_user1');
  ...
  describe('removeCompletelyByRootGroupId', () => {
    describe('when action is public', () => {
      test('Should remove the group and its descendants and publicize pages that are only visible to the groups to be removed', async () => {
        ...
        expect(normalizeGrantedGroups(cannotBePublicized?.grantedGroups)).toEqual(
          expect.arrayContaining([
            { item: groupId13, type: GroupType.userGroup },
            { item: groupId15, type: GroupType.userGroup },
          ]),
        ...
        expect(normalizeGrantedGroups(cannotBePublicizedAfterDeleteProcess?.grantedGroups)).toEqual(
          expect.arrayContaining([{ item: groupId15, type: GroupType.userGroup }]),
        );
