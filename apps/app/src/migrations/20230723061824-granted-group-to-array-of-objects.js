...
module.exports = {
  async up(db, client) {
    ...
    await pageOperationCollection.updateMany({ 'options.grantUserGroupId': { $ne: null } }, [
      {
        $set: {
    ...
    await pageOperationCollection.updateMany({ 'page.grantedGroup': { $ne: null } }, [
      {
        $set: {
          'page.grantedGroup': [
            {
    ...
    await pageOperationCollection.updateMany({ 'exPage.grantedGroup': { $ne: null } }, [
      {
        $set: {
          'exPage.grantedGroup': [
            {
  ...
  async down(db, client) {
    ...
    await pageOperationCollection.updateMany({ 'options.grantUserGroupIds': { $exists: true } }, [
      {
        $set: {
    ...
    await pageOperationCollection.updateMany({ 'page.grantedGroups': { $exists: true } }, [
      {
        $set: {
    ...
    await pageOperationCollection.updateMany({ 'exPage.grantedGroups': { $exists: true } }, [
      {
        $set: {
