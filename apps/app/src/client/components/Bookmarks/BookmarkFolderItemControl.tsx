...
export const BookmarkFolderItemControl: React.FC<{
  ...
  onClickRename: () => void;
  onClickDelete: () => void;
}> = ({ children, onClickMoveToRoot, onClickRename, onClickDelete }): JSX.Element => {
    ...
    <Dropdown isOpen={isOpen} toggle={() => setIsOpen(!isOpen)}>
      ...
      {isOpen && (
        <DropdownMenu container="body" style={{ zIndex: 1055 }}>
          {onClickMoveToRoot && (
            <DropdownItem onClick={onClickMoveToRoot} className="grw-page-control-dropdown-item">
              <span className="material-symbols-outlined grw-page-control-dropdown-icon">bookmark</span>
              {t('bookmark_folder.move_to_root')}
            </DropdownItem>
          )}
          <DropdownItem onClick={onClickRename} className="grw-page-control-dropdown-item">
          ...
          <DropdownItem className="pt-2 grw-page-control-dropdown-item text-danger" onClick={onClickDelete}>
