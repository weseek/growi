...
export const PageAccessoriesControl = memo((props: Props): JSX.Element => {
  ...
  return (
    <button type="button" className={`btn btn-outline-neutral-secondary ${moduleClass} ${className} rounded-pill py-1 px-lg-3`} onClick={onClick}>
      ...
      <span className="grw-labels d-none d-lg-flex">
        ...
        {count != null ? <CountBadge count={count} offset={offset} /> : <div className="px-2"></div>}
      </span>
    </button>
  );
});
