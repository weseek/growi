...
export const ResizableArea = memo((props: ResizableAreaProps): JSX.Element => {
  const { className, width, minWidth = 0, disabled, children, onResize, onResizeDone, onCollapsed } = props;
  ...
  return (
    <>
      <div ref={resizableContainer} className={`${styles['grw-resizable-area']} ${className}`} style={{ width }}>
        {children}
      </div>
      <div className={styles['grw-navigation-draggable']}>
        {!disabled && (
          <>
            <div className="grw-navigation-draggable-hitarea" onMouseDown={dragableAreaMouseDownHandler} />
