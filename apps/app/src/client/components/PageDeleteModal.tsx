...
import type { HasObjectId, IPageInfoForEntity, IPageToDeleteWithMeta, IDataWithMeta } from '@growi/core';
...
const PageDeleteModal: FC = () => {
  ...
  const notOperatablePages: IPageToDeleteWithMeta[] = (deleteModalData?.pages ?? []).filter((p) => !isIPageInfoForEntityForDeleteModal(p.meta));
  ...
  async function deletePage() {
    ...
    if (deleteModalData.pages.length > 1) {
      try {
        ...
        deleteModalData.pages.forEach((p) => {
    ...
    if (pages != null) {
      return pages.map((page) => (
        <p key={page.data._id} className="mb-1">
          ...
          {page.meta?.isDeletable != null && !page.meta.isDeletable && (
