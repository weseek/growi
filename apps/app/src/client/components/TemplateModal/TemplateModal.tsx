...
import { extractSupportedLocales, getLocalizedTemplate, type TemplateSummary } from '@growi/pluginkit/dist/v4';
...
const TemplateListGroupItem: React.FC<TemplateSummaryItemProps> = ({ templateSummary, onClick, isSelected, usersDefaultLang }) => {
  ...
  return (
    <a className={`list-group-item list-group-item-action ${isSelected ? 'active' : ''}`} onClick={onClick} aria-current="true">
      ...
      {templateLocales != null &&
        Array.from(templateLocales).map((locale) => (
          <span key={locale} className="badge border rounded-pill text-muted me-1">
            {locale}
          </span>
        ))}
    </a>
...
const TemplateDropdownItem: React.FC<TemplateSummaryItemProps> = ({ templateSummary, onClick, usersDefaultLang }) => {
  const localizedTemplate = getLocalizedTemplate(templateSummary, usersDefaultLang);
  const templateLocales = extractSupportedLocales(templateSummary);
              ...
              <DropdownToggle caret type="button" outline className="w-100 text-end" disabled={isLoading}>
                <span className="float-start">
                  {(() => {
                    ...
                    return selectedLocalizedTemplate != null && selectedLocalizedTemplate.isValid ? selectedLocalizedTemplate.title : t('Select template');
                  })()}
                </span>
              </DropdownToggle>
                  ...
                  <DropdownMenu className="dropdown-menu" role="menu">
                    {selectedTemplateLocales != null &&
                      Array.from(selectedTemplateLocales).map((locale) => {
                        return (
                          <DropdownItem data-testid="select-locale-dropdown-item" key={locale} onClick={() => setSelectedTemplateLocale(locale)}>
                            <span>{locale}</span>
                          </DropdownItem>
                        );
                      })}
...
export const TemplateModal = (): JSX.Element => {
  ...
  return (
    <Modal className="link-edit-modal" isOpen={templateModalStatus.isOpened} toggle={close} size="xl" autoFocus={false}>
      {templateModalStatus.isOpened && <TemplateModalSubstance templateModalStatus={templateModalStatus} close={close} />}
    </Modal>
  );
};
