  ...
  const renderAdminMenu = useCallback(() => {
    return (
      <>
        ...
        <li>{user.admin ? <RevokeAdminMenuItem user={user} /> : <GrantAdminButton user={user} />}</li>
        <li>{user.readOnly ? <RevokeReadOnlyMenuItem user={user} /> : <GrantReadOnlyButton user={user} />}</li>
      </>
    );
  }, [t, user]);
...
const UserMenuWrapper: React.ForwardRefExoticComponent<Pick<any, string | number | symbol> & React.RefAttributes<any>> = withUnstatedContainers(UserMenu, [
  AdminUsersContainer,
]);
