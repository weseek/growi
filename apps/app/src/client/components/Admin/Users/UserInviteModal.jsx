    ...
    return (
      <>
        <div className="col text-start form-check form-check-info" onChange={this.handleCheckBox}>
          <input type="checkbox" id="sendEmail" className="form-check-input" name="sendEmail" defaultChecked={this.state.sendEmail} disabled={!isMailerSetup} />
        ...
        <div>
          <button type="button" className="btn btn-outline-secondary me-2" onClick={this.onToggleModal}>
            {t('Cancel')}
          </button>

          <button type="button" className="btn btn-primary" onClick={this.handleSubmit} disabled={!this.validEmail() || isCreateUserButtonPushed}>
  ...
  renderCreatedModalFooter() {
    ...
    return (
      <>
        ...
        <button type="button" className="btn btn-outline-secondary" onClick={this.onToggleModal}>
          {t('Close')}
        </button>
      </>
    );
  ...
  render() {
    ...
    return (
      <Modal isOpen={adminUsersContainer.state.isUserInviteModalShown}>
        ...
        <ModalBody>{invitedEmailList == null ? this.renderModalBody() : this.renderCreatedModalBody()}</ModalBody>
        <ModalFooter className="d-flex">{invitedEmailList == null ? this.renderModalFooter() : this.renderCreatedModalFooter()}</ModalFooter>
      </Modal>
    );
  }
}
