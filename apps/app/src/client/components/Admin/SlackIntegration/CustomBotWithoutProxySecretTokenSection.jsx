...
const CustomBotWithoutProxySecretTokenSection = (props) => {
  const { slackSigningSecret, slackBotToken, slackSigningSecretEnv, slackBotTokenEnv, onUpdatedSecretToken } = props;
    ...
    <div className="w-75 mx-auto">
      ...
      <div className="row">
        <div className="col-sm">
          <p>Database</p>
          <input className="form-control" type="text" value={inputSigningSecret} onChange={(e) => setInputSigningSecret(e.target.value)} />
        </div>

        <div className="col-sm">
          <p>Environment variables</p>
          <input className="form-control" type="text" defaultValue={slackSigningSecretEnv} readOnly />
          <p className="form-text text-muted">
            {/* eslint-disable-next-line max-len, react/no-danger */}
            <small
              dangerouslySetInnerHTML={{ __html: t('admin:slack_integration.use_env_var_if_empty', { variable: 'SLACKBOT_WITHOUT_PROXY_SIGNING_SECRET' }) }}
            />
          </p>
        </div>
      </div>
      ...
      <div className="row">
        <div className="col-sm">
          <p>Database</p>
          <input className="form-control" type="text" value={inputBotToken} onChange={(e) => setInputBotToken(e.target.value)} />
        </div>

        <div className="col-sm">
          <p>Environment variables</p>
          <input className="form-control" type="text" defaultValue={slackBotTokenEnv} readOnly />
          <p className="form-text text-muted">
