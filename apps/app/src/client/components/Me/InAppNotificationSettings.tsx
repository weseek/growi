...
const isCheckedRule = (ruleName: string, subscribeRules: SubscribeRule[]) =>
  subscribeRules.find((stateRule) => stateRule.name === ruleName)?.isEnabled || false;
    ...
    <>
      ...
      <div className="row">
        <div className="offset-md-3 col-md-6 text-start">
          {subscribeRulesMenuItems.map((rule) => (
            <div key={rule.name} className="form-check form-switch form-check-success">
              <input
                type="checkbox"
              ...
              <p className="form-text text-muted small">{t(rule.description)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="row my-3">
        <div className="offset-4 col-5">
          <button data-testid="grw-in-app-notification-settings-update-button" type="button" className="btn btn-primary" onClick={updateSettingsHandler}>
            {t('Update')}
          </button>
        </div>
      </div>
