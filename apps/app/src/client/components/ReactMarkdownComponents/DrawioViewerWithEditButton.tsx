...
import { DrawioViewer, type DrawioEditByViewerProps, type DrawioViewerProps } from '@growi/remark-drawio';
...
export const DrawioViewerWithEditButton = React.memo((props: DrawioViewerProps): JSX.Element => {
  ...
  const showEditButton = isNoEditingUsers && !isRevisionOutdated && isRendered && !isGuestUser && !isReadOnlyUser && !isSharedUser && shareLinkId == null;

  return (
    <div className={`drawio-viewer-with-edit-button ${styles['drawio-viewer-with-edit-button']}`}>
      {showEditButton && (
        <button type="button" className="btn btn-sm btn-outline-secondary btn-edit-drawio" onClick={editButtonClickHandler}>
