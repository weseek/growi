...
const PageDuplicateModal = (): JSX.Element => {
  ...
  const renderBodyContent = () => {
    ...
    return (
      <>
        <div className="mt-3">
          <label className="form-label">{t('modal_duplicate.label.Current page name')}</label>
          <br />
          <code>{path}</code>
        </div>
        <div className="mt-3">
          ...
          <div className="input-group">
            ...
            <div className="flex-fill">
              {isReachable ? (
                <PagePathAutoComplete initializedPath={path} onSubmit={duplicate} onInputChange={ppacInputChangeHandler} autoFocus />
              ) : (
        ...
        <div className="mt-3">
          {isDuplicateRecursively && existingPaths.length !== 0 && <DuplicatePathsTable existingPaths={existingPaths} fromPath={path} toPath={pageNameInput} />}
        </div>
      </>
    );
  };
  ...
  const renderFooterContent = () => {
    ...
    const submitButtonEnabled = existingPaths.length === 0 || (isDuplicateRecursively && isDuplicateRecursivelyWithoutExistPath);

    return (
      <>
        <ApiErrorMessageList errs={errs} targetPath={pageNameInput} />
        <button type="button" className="btn btn-primary" data-testid="btn-duplicate" onClick={duplicate} disabled={!submitButtonEnabled}>
