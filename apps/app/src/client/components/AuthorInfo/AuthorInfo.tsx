...
export const AuthorInfo = (props: AuthorInfoProps): JSX.Element => {
  ...
  const infoLabelForPageSide = mode === 'create' ? t('author_info.created_by') : t('author_info.updated_by');
  const nullinfoLabelForFooter = mode === 'create' ? 'Created by' : 'Updated by';
  const infoLabelForFooter = mode === 'create' ? t('author_info.created_at') : t('author_info.last_revision_posted_at');
  ...
  if (locate === 'footer') {
    try {
      return (
        <p>
          {infoLabelForFooter} {format(new Date(date), formatType)} by <UserPicture user={user} size="sm" /> {userLabel}
        </p>
      );
    } catch (err) {
      if (err instanceof RangeError) {
        return (
          <p>
            {nullinfoLabelForFooter} <UserPicture user={user} size="sm" /> {userLabel}
          </p>
        );
      }
      return <></>;
