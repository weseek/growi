...
const PasswordResetRequestForm: FC = () => {
  ...
  return (
    <form onSubmit={sendPasswordResetRequestMail}>
      {!isMailerSetup ? (
        <div className="alert alert-danger">{t('forgot_password.please_enable_mailer_alert')}</div>
      ) : (
        <>
          {/* lock-icon large */}
          <h1>
          ...
          <div>
            <button className="btn btn-lg btn-primary" type="submit" disabled={!isMailerSetup}>
