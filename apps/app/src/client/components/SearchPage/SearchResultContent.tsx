...
const PageContentFooter = dynamic(() => import('~/components/PageView/PageContentFooter').then((mod) => mod.PageContentFooter), { ssr: false });
...
export const SearchResultContent: FC<Props> = (props: Props) => {
  ...
  const { pageWithMeta, highlightKeywords, showPageControlDropdown, forceHideMenuItems } = props;
  ...
  const RightComponent = useCallback(() => {
    ...
    const additionalMenuItemRenderer = revisionId != null ? (props) => <AdditionalMenuItems {...props} pageId={page._id} revisionId={revisionId} /> : undefined;
  ...
  }, [page, shouldExpandContent, showPageControlDropdown, forceHideMenuItems, duplicateItemClickedHandler, renameItemClickedHandler, deleteItemClickedHandler]);

  const fluidLayoutClass = shouldExpandContent ? _fluidLayoutClass : '';

  return (
    <div key={page._id} data-testid="search-result-content" className={`dynamic-layout-root ${moduleClass} ${fluidLayoutClass}`}>
      <RightComponent />
      ...
      >
        {page.revision != null && rendererOptions != null && <RevisionLoader rendererOptions={rendererOptions} pageId={page._id} revisionId={page.revision} />}
        {page.revision != null && (
          <PageComment rendererOptions={rendererOptions} pageId={page._id} pagePath={page.path} revision={page.revision} currentUser={currentUser} isReadOnly />
        )}
