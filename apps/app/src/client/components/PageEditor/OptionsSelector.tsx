...
import { type EditorTheme, type KeyMapMode, PasteMode, AllPasteMode, DEFAULT_KEYMAP, DEFAULT_PASTE_MODE, DEFAULT_THEME } from '@growi/editor';
import { useTranslation } from 'next-i18next';
import Image from 'next/image';
import { Dropdown, DropdownToggle, DropdownMenu, Input, FormGroup } from 'reactstrap';
...
const RadioListItem = (props: RadioListItemProps): JSX.Element => {
  ...
  return (
    <li className="list-group-item border-0 d-flex align-items-center">
      <input onClick={onClick} className="form-check-input me-3" type="radio" name="listGroupRadio" id={`editor_config_radio_item_${text}`} checked={checked} />
      {icon}
      <label className="form-check-label stretched-link fs-6" htmlFor={`editor_config_radio_item_${text}`}>
        {text}
      </label>
    </li>
  );
...
const ThemeSelector = memo(({ onClickBefore }: { onClickBefore: () => void }): JSX.Element => {
    ...
    () => (
      <>
        {(Object.keys(EDITORTHEME_LABEL_MAP) as EditorTheme[]).map((theme) => {
          ...
          return <RadioListItem onClick={() => update({ theme })} text={themeLabel} checked={theme === selectedTheme} />;
        })}
      </>
    ),
  ...
  return <Selector header={t('page_edit.theme')} onClickBefore={onClickBefore} items={listItems} />;
});
...
const KeymapSelector = memo(({ onClickBefore }: { onClickBefore: () => void }): JSX.Element => {
    ...
    () => (
      <>
        {(Object.keys(KEYMAP_LABEL_MAP) as KeyMapMode[]).map((keymapMode) => {
          ...
          const icon =
            keymapMode !== 'default' ? <Image src={`/images/icons/${keymapMode}.png`} width={16} height={16} className="me-2" alt={keymapMode} /> : null;
          return <RadioListItem onClick={() => update({ keymapMode })} icon={icon} text={keymapLabel} checked={keymapMode === selectedKeymapMode} />;
        })}
      </>
    ),
  ...
  return <Selector header={t('page_edit.keymap')} onClickBefore={onClickBefore} items={listItems} />;
});
...
const IndentSizeSelector = memo(({ onClickBefore }: { onClickBefore: () => void }): JSX.Element => {
    ...
    () => (
      <>
        {TYPICAL_INDENT_SIZE.map((indent) => {
          return <RadioListItem onClick={() => mutateCurrentIndentSize(indent)} text={indent.toString()} checked={indent === currentIndentSize} />;
        })}
      </>
    ),
  ...
  return <Selector header={t('page_edit.indent')} onClickBefore={onClickBefore} items={listItems} />;
});
...
const PasteSelector = memo(({ onClickBefore }: { onClickBefore: () => void }): JSX.Element => {
  ...
  return <Selector header={t('page_edit.paste.title')} onClickBefore={onClickBefore} items={listItems} />;
});
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isDeviceLargerThanMd ? <label className="ms-1 me-1">{t('page_edit.editor_config')}</label> : <></>}
      </DropdownToggle>
      <DropdownMenu container="body">
        {status === OptionsStatus.Home && (
          <div className="d-flex flex-column">
            <label className="text-muted ms-3">{t('page_edit.editor_config')}</label>
            <hr className="my-1" />
            <ChangeStateButton
        ...
        {status === OptionsStatus.Theme && <ThemeSelector onClickBefore={() => setStatus(OptionsStatus.Home)} />}
        {status === OptionsStatus.Keymap && <KeymapSelector onClickBefore={() => setStatus(OptionsStatus.Home)} />}
        {status === OptionsStatus.Indent && <IndentSizeSelector onClickBefore={() => setStatus(OptionsStatus.Home)} />}
        {status === OptionsStatus.Paste && <PasteSelector onClickBefore={() => setStatus(OptionsStatus.Home)} />}
      </DropdownMenu>
    </Dropdown>
  );
};
