...
import { useCurrentProductNavWidth, useCurrentSidebarContents, usePreferCollapsedMode } from '~/stores/ui';
...
export const getServerSideCommonProps: GetServerSideProps<CommonProps> = async (context: GetServerSidePropsContext) => {
  ...
  const { appService, configManager, customizeService, attachmentService } = crowi;
  ...
  const userUISettings = user != null && UserUISettings != null ? await UserUISettings.findOne({ user: user._id }).exec() : req.session.uiSettings; // for guests
...
export const getLangAtServerSide = (req: CrowiRequest): Lang => {
  ...
  return user == null ? detectLocaleFromBrowserAcceptLanguage(headers) : (user.lang ?? configManager.getConfig('app:globalLang') ?? Lang.en_US ?? Lang.en_US);
};
...
export const generateCustomTitle = (props: CommonProps, title: string): string => {
  return props.customTitleTemplate.replace('{{sitename}}', props.appTitle).replace('{{pagepath}}', title).replace('{{pagename}}', title);
};
...
export const generateCustomTitleForPage = (props: CommonProps, pagePath: string): string => {
  ...
  return props.customTitleTemplate.replace('{{sitename}}', props.appTitle).replace('{{pagepath}}', pagePath).replace('{{pagename}}', dPagePath.latter);
};
