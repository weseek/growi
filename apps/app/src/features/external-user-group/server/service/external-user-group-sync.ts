...
import type { ExternalGroupProviderType, ExternalUserGroupTreeNode, ExternalUserInfo, IExternalUserGroupHasId } from '../../interfaces/external-user-group';
...
abstract class ExternalUserGroupSyncService implements S2sMessageHandlable {
  ...
  async syncExternalUserGroups(): Promise<void> {
    ...
    try {
      ...
      const totalCount = trees.map((tree) => this.getGroupCountOfTree(tree)).reduce((sum, current) => sum + current);
  ...
  private async createUpdateExternalUserGroup(node: ExternalUserGroupTreeNode, parentId?: string): Promise<IExternalUserGroupHasId> {
    const externalUserGroup = await ExternalUserGroup.findAndUpdateOrCreateGroup(node.name, node.id, this.groupProviderType, node.description, parentId);
  ...
  getGroupCountOfTree(tree: ExternalUserGroupTreeNode): number {
    if (tree.childGroupNodes.length === 0) {
      return 1;
    }
