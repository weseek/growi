...
describe('ExternalUserGroup model', () => {
  describe('findAndUpdateOrCreateGroup', () => {
    ...
    it('creates new group with parent', async () => {
      ...
      const newGroup = await ExternalUserGroup.findAndUpdateOrCreateGroup('new group', 'nonExistentExternalId', 'testProvider', undefined, groupId.toString());
    ...
    it('throws error when parent does not exist', async () => {
      try {
        await ExternalUserGroup.findAndUpdateOrCreateGroup('new group', 'nonExistentExternalId', 'testProvider', undefined, new mongoose.Types.ObjectId());
      } catch (e) {
