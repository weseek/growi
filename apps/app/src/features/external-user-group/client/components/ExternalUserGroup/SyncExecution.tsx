...
export const SyncExecution = ({ provider, requestSyncAPI, AdditionalForm = () => <></> }: SyncExecutionProps): JSX.Element => {
  ...
  const renderProgressBar = () => {
    if (syncStatus === SyncStatus.beforeSync) {
      return null;
    }
    ...
    return <LabeledProgressBar header={header} currentCount={progress.current} totalCount={progress.total} />;
  };
  ...
  return (
    <>
      ...
      <Modal isOpen={isAlertModalOpen} toggle={() => setIsAlertModalOpen(false)}>
        ...
        <ModalBody>
          ...
          <div className="text-center">
            <button className="btn btn-primary" type="button" onClick={onSyncExecConfirmBtnClick}>
              {t('Execute')}
            </button>
          </div>
        </ModalBody>
