...
export const useSWRxExternalUserGroup = (groupId: string | null): SWRResponse<IExternalUserGroupHasId, Error> => {
  return useSWRImmutable(groupId != null ? `/external-user-groups/${groupId}` : null, (endpoint) => apiv3Get(endpoint).then((result) => result.data.userGroup));
};

export const useSWRxExternalUserGroupList = (initialData?: IExternalUserGroupHasId[]): SWRResponse<IExternalUserGroupHasId[], Error> => {
  return useSWRImmutable('/external-user-groups', (endpoint) => apiv3Get(endpoint, { pagination: false }).then((result) => result.data.userGroups), {
...
export const useSWRxChildExternalUserGroupList = (
  ...
  const swrResponse = useSWRImmutable(
    ...
    ([endpoint, parentIds, includeGrandChildren]) =>
      apiv3Get<ChildUserGroupListResult<IExternalUserGroupHasId>>(endpoint, { parentIds, includeGrandChildren }).then((result) => result.data),
  );
...
export const useSWRxExternalUserGroupRelations = (groupId: string | null): SWRResponse<IUserGroupRelationHasIdPopulatedUser[], Error> => {
  return useSWRImmutable(groupId != null ? `/external-user-groups/${groupId}/external-user-group-relations` : null, (endpoint) =>
...
export const useSWRxExternalUserGroupRelationList = (
  ...
  return useSWRImmutable(
    ...
    ([endpoint, groupIds, childGroupIds]) =>
      apiv3Get<UserGroupRelationListResult<IExternalUserGroupRelationHasId>>(endpoint, { groupIds, childGroupIds }).then(
...
export const useSWRxAncestorExternalUserGroups = (groupId: string | null): SWRResponse<IExternalUserGroupHasId[], Error> => {
  return useSWRImmutable(groupId != null ? ['/external-user-groups/ancestors', groupId] : null, ([endpoint, groupId]) =>
    apiv3Get(endpoint, { groupId }).then((result) => result.data.ancestorUserGroups),
  );
};
