/**
 * Integration tests for PUT /_api/v3/inline-comments/:id (task 2.1).
 *
 * Same passthrough-auth pattern as resolve.integ.ts — see that file's header.
 *
...
const FIXTURE_ROOT = '/inline-comment-update-route-integ';
const creatorUsername = 'inline-comment-update-route-integ-creator';
const otherUsername = 'inline-comment-update-route-integ-other';
const readOnlyUsername = 'inline-comment-update-route-integ-readonly';

describe('PUT /_api/v3/inline-comments/:id', () => {
  ...
  it('returns 400 when a read-only user (not allowed to comment) attempts the edit', async () => {
    ...
    const res = await request(readOnlyApp)
      .put(`/_api/v3/inline-comments/${originCommentId}`)
  ...
  it('returns 400 when :id is a reply (not an origin comment)', async () => {
    const res = await request(app)
      .put(`/_api/v3/inline-comments/${replyCommentId}`)
      .send({ comment: 'edited' });
  ...
  it('returns 404 when :id does not exist', async () => {
    const res = await request(app)
      .put(`/_api/v3/inline-comments/${new Types.ObjectId()}`)
      .send({ comment: 'edited' });
  ...
  it('updates the comment body and returns the updated inlineComment (200)', async () => {
    const res = await request(app)
      .put(`/_api/v3/inline-comments/${originCommentId}`)
      .send({ comment: 'edited by the creator' });

    expect(res.status).toBe(200);
    expect(res.body.inlineComment.id).toBe(originCommentId);
    expect(res.body.inlineComment.comment).toBe('edited by the creator');
  });
