...
import { type FC, memo, useRef, useEffect, useState, useCallback } from 'react';
...
const AiAssistantChatSidebarSubstance: React.FC<AiAssistantChatSidebarSubstanceProps> = (props: AiAssistantChatSidebarSubstanceProps) => {
  const { aiAssistantData, threadData, closeAiAssistantChatSidebar } = props;
  ...
  useEffect(() => {
    const fetchAndSetMessageData = async () => {
      ...
      if (messageData != null) {
        const normalizedMessageData = messageData.data.reverse().filter((message) => message.metadata?.shouldHideMessage !== 'true');
    ...
    <>
      <div className="d-flex flex-column vh-100">
        <div className="d-flex align-items-center p-3 border-bottom position-sticky top-0 bg-body z-1">
          ...
          <button type="button" className="btn btn-link p-0 border-0" onClick={closeAiAssistantChatSidebar}>
          ...
          {currentThreadId != null ? (
            <div className="vstack gap-4 pb-2">
              {messageLogs.map((message) => (
                <MessageCard key={message.id} role={message.isUserMessage ? 'user' : 'assistant'}>
                  {message.content}
                </MessageCard>
              ))}
              {generatingAnswerMessage != null && <MessageCard role="assistant">{generatingAnswerMessage.content}</MessageCard>}
            ...
            <>
              <p className="fs-6 text-body-secondary mb-0">{aiAssistantData.description}</p>

              <div>
                <p className="text-body-secondary">{t('sidebar_aichat.instruction_label')}</p>
                <div className="card bg-body-tertiary border-0">
                  <div className="card-body p-3">
                    <p className="fs-6 text-body-secondary mb-0">{aiAssistantData.additionalInstruction}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="d-flex flex-column gap-1">
                  {aiAssistantData.pagePathPatterns.map((pagePathPattern) => (
                    <a key={pagePathPattern} href="#" className="fs-6 text-body-secondary text-decoration-none">
                      {pagePathPattern}
                    </a>
                  ))}
                </div>
          ...
          <div className="mt-auto">
            <form onSubmit={form.handleSubmit(submit)} className="flex-fill vstack gap-3">
              <div className="flex-fill hstack gap-2 align-items-end m-0">
                ...
                />
                <button type="submit" className="btn btn-submit no-border" disabled={form.formState.isSubmitting || isGenerating}>
              ...
              <div className="form-check form-switch">
                ...
                {/* Help */}
                <a id="tooltipForHelpOfSummaryMode" role="button" className="ms-1">
                  <span className="material-symbols-outlined fs-6" style={{ lineHeight: 'unset' }}>
                    help
                  </span>
                </a>
                <UncontrolledTooltip target="tooltipForHelpOfSummaryMode">{t('sidebar_aichat.summary_mode_help')}</UncontrolledTooltip>
              </div>
            </form>
            ...
            {form.formState.errors.input != null && (
              <div className="mt-4 bg-danger bg-opacity-10 rounded-3 p-2 w-100">
                ...
                >
                  <span className={`material-symbols-outlined mt-2 me-1 ${isErrorDetailCollapsed ? 'rotate-90' : ''}`}>chevron_right</span>
                ...
                <Collapse isOpen={isErrorDetailCollapsed}>
                  <div className="ms-2">
                    <div className="">
                      <div className="text-body-secondary small">{form.formState.errors.input?.message}</div>
                    </div>
                  </div>
                </Collapse>
              </div>
...
export const AiAssistantChatSidebar: FC = memo((): JSX.Element => {
  ...
  return (
    ...
    >
      <SimpleBar scrollableNodeProps={{ ref: sidebarScrollerRef }} className="h-100 position-relative" autoHide>
        <AiAssistantChatSidebarSubstance threadData={threadData} aiAssistantData={aiAssistantData} closeAiAssistantChatSidebar={closeAiAssistantChatSidebar} />
      </SimpleBar>
    </div>
  );
});
