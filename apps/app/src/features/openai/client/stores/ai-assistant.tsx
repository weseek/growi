...
import type { AccessibleAiAssistantsHasId, AiAssistantHasId } from '../../interfaces/ai-assistant';
...
export const useAiAssistantManagementModal = (
  ...
  return {
    ...
    open: useCallback(
      (aiAssistantData) => {
        swrResponse.mutate({ isOpened: true, aiAssistantData });
      },
      [swrResponse],
    ),
...
export const useSWRxAiAssistants = (): SWRResponse<AccessibleAiAssistantsHasId, Error> => {
  return useSWRImmutable<AccessibleAiAssistantsHasId>(['/openai/ai-assistants'], ([endpoint]) =>
    apiv3Get(endpoint).then((response) => response.data.accessibleAiAssistants),
  );
};
...
type AiAssistantChatSidebarUtils = {
  open(aiAssistantData: AiAssistantHasId, threadData?: IThreadRelationHasId): void;
  close(): void;
};
