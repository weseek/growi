...
export const useSWRxUserGroup = (groupId: string | null): SWRResponse<IUserGroupHasId, Error> => {
  return useSWRImmutable(groupId != null ? `/user-groups/${groupId}` : null, (endpoint) =>
    apiv3Get<UserGroupResult>(endpoint).then((result) => result.data.userGroup),
  );
};
...
export const useSWRxUserGroupList = (initialData?: IUserGroupHasId[], isExternalGroup = false): SWRResponse<IUserGroupHasId[], Error> => {
  ...
  return useSWRImmutable(url, (endpoint) => apiv3Get<UserGroupListResult>(endpoint, { pagination: false }).then((result) => result.data.userGroups), {
...
export const useSWRxChildUserGroupList = (
  ...
  const swrResponse = useSWRImmutable(
    ...
    ([endpoint, parentIds, includeGrandChildren]) =>
      apiv3Get<ChildUserGroupListResult>(endpoint, { parentIds, includeGrandChildren }).then((result) => result.data),
  );
...
export const useSWRxUserGroupRelations = (groupId: string | null): SWRResponse<IUserGroupRelationHasIdPopulatedUser[], Error> => {
  return useSWRImmutable(groupId != null ? `/user-groups/${groupId}/user-group-relations` : null, (endpoint) =>
    apiv3Get<UserGroupRelationsResult>(endpoint).then((result) => result.data.userGroupRelations),
  );
};
...
export const useSWRxUserGroupRelationList = (
  ...
  return useSWRImmutable(
    ...
    ([endpoint, groupIds, childGroupIds]) =>
      apiv3Get<UserGroupRelationListResult>(endpoint, { groupIds, childGroupIds }).then((result) => result.data.userGroupRelations),
    {
      fallbackData: initialData,
    },
  );
...
export const useSWRxUserGroupPages = (groupId: string | undefined, limit: number, offset: number): SWRResponse<IPageHasId[], Error> => {
  return useSWRImmutable(groupId != null ? [`/user-groups/${groupId}/pages`, limit, offset] : null, ([endpoint, limit, offset]) =>
    apiv3Get<UserGroupPagesResult>(endpoint, { limit, offset }).then((result) => result.data.pages),
  );
};

export const useSWRxSelectableParentUserGroups = (groupId: string | null): SWRResponse<IUserGroupHasId[], Error> => {
  return useSWRImmutable(groupId != null ? ['/user-groups/selectable-parent-groups', groupId] : null, ([endpoint, groupId]) =>
    apiv3Get<SelectableParentUserGroupsResult>(endpoint, { groupId }).then((result) => result.data.selectableParentGroups),
  );
};

export const useSWRxSelectableChildUserGroups = (groupId: string | null): SWRResponse<IUserGroupHasId[], Error> => {
  return useSWRImmutable(groupId != null ? ['/user-groups/selectable-child-groups', groupId] : null, ([endpoint, groupId]) =>
    apiv3Get<SelectableUserChildGroupsResult>(endpoint, { groupId }).then((result) => result.data.selectableChildGroups),
  );
};

export const useSWRxAncestorUserGroups = (groupId: string | null): SWRResponse<IUserGroupHasId[], Error> => {
  return useSWRImmutable(groupId != null ? ['/user-groups/ancestors', groupId] : null, ([endpoint, groupId]) =>
    apiv3Get<AncestorUserGroupsResult>(endpoint, { groupId }).then((result) => result.data.ancestorUserGroups),
  );
};
