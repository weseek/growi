...
export const useSWRxInAppNotificationStatus = (): SWRResponse<number, Error> => {
  return useSWR('/in-app-notification/status', (endpoint) => apiv3Get(endpoint).then((response) => response.data.count));
};
