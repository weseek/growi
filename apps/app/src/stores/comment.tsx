...
export const useSWRxPageComment = (pageId: Nullable<string>): SWRResponse<ICommentHasIdList, Error> & CommentOperation => {
  ...
  const swrResponse = useSWR(shouldFetch ? ['/comments.get', pageId] : null, ([endpoint, pageId]) =>
    apiGet(endpoint, { page_id: pageId }).then((response: IResponseComment) => response.comments),
  );
