import type { HasObjectId, IUser, IPage } from '@growi/core';
...
export default class InAppNotificationService {
  ...
  emitSocketIo = async (targetUsers) => {
    if (this.socketIoService.isInitialized) {
      targetUsers.forEach(async (userId) => {
        ...
        await this.socketIoService.getDefaultSocket().in(getRoomNameWithId(RoomPrefix.USER, userId)).emit('notificationUpdated');
      });
    }
  };

  upsertByActivity = async (users: Types.ObjectId[], activity: ActivityDocument, snapshot: string, createdAt?: Date | null): Promise<void> => {
  ...
  getLatestNotificationsByUser = async (
    ...
    try {
      ...
      const paginationResult = await (InAppNotification as any).paginate(paginateOptions, {
        sort: { createdAt: -1 },
        limit,
        offset,
        populate: [
