import type { IPageInfoForListing, IPageInfo, IPage } from '@growi/core';
...
const validator = {
  ...
  pageIdOrPathRequired: oneOf([query('id').isMongoId(), query('path').isString()], 'id or path is required'),
  pageIdsOrPathRequired: [
    ...
    // existence check
    oneOf([query('pageIds').exists(), query('path').exists()], 'pageIds or path is required'),
  ],
  infoParams: [query('attachBookmarkCount').isBoolean().optional(), query('attachShortBody').isBoolean().optional()],
};
...
const routerFactory = (crowi: Crowi): Router => {
  ...
  router.get(
    ...
    accessTokenParser,
    loginRequired,
    ...validator.pagePathRequired,
    apiV3FormValidator,
    async (req: AuthorizedRequest, res: ApiV3Response): Promise<any> => {
  ...
  router.get(
    ...
    apiV3FormValidator,
    async (req: AuthorizedRequest, res: ApiV3Response) => {
      const { pageIds, path, attachBookmarkCount: attachBookmarkCountParam, attachShortBody: attachShortBodyParam } = req.query;
