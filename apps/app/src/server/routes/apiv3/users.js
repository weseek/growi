...
export const setup = (crowi) => {
  ...
  validator.statusList = [
    query('selectedStatusList')
      ...
      .custom((value, { req }) => {
        ...
        throw new Error(
          "the param 'selectedStatusList' is not allowed to use by the users except administrators",
        );
      }),
  ...
  validator.usernames = [
    ...
    query('limit')
      .optional()
      .isInt({ max: 20 })
      .withMessage('You should set less than 20 or not to set limit.'),
    query('options')
      .optional()
      .isString()
      .withMessage('options must be string'),
  ];
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      const statusNoList = selectedStatusList.includes('all')
      ...
      try {
        ...
        return res.apiv3Err(
          new ErrorV3(msg, 'user-group-list-fetch-failed'),
          500,
        );
      }
    },
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger.error('Error', err);
        return res.apiv3Err(
          new ErrorV3(msg, 'retrieve-recent-created-pages-failed'),
          500,
        );
      }
      ...
      try {
        const result = await Page.findListByCreator(
          user,
          req.user,
          queryOptions,
        );
  ...
  router.post(
    '/invite',
    accessTokenParser([SCOPE.WRITE.ADMIN.USER_MANAGEMENT]),
    ...
    async (req, res) => {
      ...
      if (req.body.sendEmail) {
        ...
        if (sendEmail.failedToSendEmailList.length > 0) {
          failedEmailList = failedEmailList.concat(
            sendEmail.failedToSendEmailList,
          );
        }
      }
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_GRANT_ADMIN,
        });
  ...
  router.put(
    ...
    loginRequiredStrictly,
    adminRequired,
    certifyUserOperationOtherThenYourOwn,
    addActivity,
    async (req, res) => {
      const { id } = req.params;
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_REVOKE_ADMIN,
        });
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_GRANT_READ_ONLY,
        });
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_REVOKE_READ_ONLY,
        });
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_ACTIVATE,
        });
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_DEACTIVATE,
        });
  ...
  router.delete(
    ...
    async (req, res) => {
      const { id } = req.params;
      const isUsersHomepageDeletionEnabled = configManager.getConfig(
        'security:user-homepage-deletion:isEnabled',
      );
      const isForceDeleteUserHomepageOnUserDeletion = configManager.getConfig(
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_REMOVE,
        });
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        return res.apiv3Err(
          new ErrorV3(msg + err.message, 'external-account-list-fetch-failed'),
          500,
        );
      }
    },
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger.error(msg, err);
        return res.apiv3Err(
          new ErrorV3(msg + err.message, 'extenral-account-delete-failed'),
        );
      }
    },
  );
          ...
          users.map(async (user) => {
            return {
              updateOne: {
                filter: { _id: user._id },
                update: {
                  $set: { imageUrlCached: await user.generateImageUrlCached() },
                },
              },
            };
          }),
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        activityEvent.emit('update', res.locals.activity._id, {
          action: SupportedAction.ACTION_ADMIN_USERS_SEND_INVITATION_EMAIL,
        });

        return res.apiv3({
          failedToSendEmail: sendEmail.failedToSendEmailList[0],
        });
      } catch (err) {
        logger.error('Error', err);
        return res.apiv3Err(new ErrorV3(err));
      ...
      try {
        ...
        if (canIncludeMixedUsernames) {
          ...
          const allUsernames = [
            ...
            ...(data?.activitySnapshotUser?.usernames || []),
          ];
