...
export const PagePathNavLayout = (props: Props): JSX.Element => {
  const { className = '', pageId, pagePath, isWipPage, formerLink, formerLinkClassName = '', latterLink, latterLinkClassName = '', maxWidth } = props;
  ...
  return (
    <div className={`${className} ${moduleClass}`} style={{ maxWidth }}>
      ...
      <div className="d-flex align-items-center">
        <h1 className={`m-0 ${latterLinkClassName}`}>{latterLink}</h1>
        {pageId != null && !isNotFound && (
          <div className="d-flex align-items-center ms-2">
            {isWipPage && <span className="badge text-bg-secondary ms-1 me-1">WIP</span>}
