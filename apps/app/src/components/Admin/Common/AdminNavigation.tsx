...
const MenuLink = ({ menu, isRoot, isListGroupItems, isActive }: MenuLinkProps) => {
  const pageTransitionClassName = isListGroupItems ? 'list-group-item list-group-item-action rounded border-0' : 'dropdown-item px-3 py-2';

  const href = isRoot ? '/admin' : urljoin('/admin', menu);

  return (
    <Link href={href} className={`${pageTransitionClassName} ${isActive ? 'active' : ''}`}>
      <MenuLabel menu={menu} />
    </Link>
  );
};
...
export const AdminNavigation = (): JSX.Element => {
      ...
      return (
        <>
          ...
          {growiCloudUri != null && growiAppIdForGrowiCloud != null && (
            <a href={`${growiCloudUri}/my/apps/${growiAppIdForGrowiCloud}`} className="list-group-item list-group-item-action border-0 round-corner">
  ...
  return (
    <React.Fragment>
      {/* List group */}
      <div className={`list-group ${moduleClass} sticky-top d-none d-lg-block`}>{getListGroupItemOrDropdownItemList(true)}</div>
      ...
      <div className="dropdown d-block d-lg-none mb-5">
        ...
        >
          <span className="float-start">
            ...
            {isActiveMenu(['/notification', '/global-notification']) && <MenuLabel menu="notification" />}
            {isActiveMenu('/slack-integration') && <MenuLabel menu="slack-integration" />}
            {isActiveMenu('/users') && <MenuLabel menu="users" />}
            {isActiveMenu(['/user-groups', 'user-group-detail']) && <MenuLabel menu="user-groups" />}
