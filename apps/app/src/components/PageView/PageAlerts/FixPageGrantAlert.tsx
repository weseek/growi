...
const FixPageGrantModal = (props: ModalProps): JSX.Element => {
  ...
  const { isOpen, pageId, dataApplicableGrant, currentAndParentPageGrantData, close } = props;
  ...
  const renderModalBodyAndFooter = () => {
    ...
    if (!isGrantAvailable) {
      return <p className="m-5">{t('fix_page_grant.modal.no_grant_available')}</p>;
    }
                ...
                <div className="dropdown ms-2">
                  ...
                  >
                    <span className="float-start ms-2">
                      {selectedGroups.length === 0 ? t('fix_page_grant.modal.select_group_default_text') : selectedGroups.map((g) => g.item.name).join(', ')}
                    </span>
                  </button>
                </div>
              </div>
              {shouldShowModalAlert && <p className="alert alert-warning">{t('fix_page_grant.modal.alert_message')}</p>}
            </div>
          </div>
        </ModalBody>
        <ModalFooter>
  ...
  return (
    <>
      ...
      {applicableGroups != null && (
        <Modal isOpen={isGroupSelectModalShown} toggle={() => setIsGroupSelectModalShown(false)}>
          ...
          <ModalBody>
            <>
              ...
              <button type="button" className="btn btn-primary mt-2 float-right" onClick={() => setIsGroupSelectModalShown(false)}>
                {t('Done')}
              </button>
            </>
          </ModalBody>
