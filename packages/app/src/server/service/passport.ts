...
class PassportService implements S2sMessageHandlable {
  ...
  getSetupStrategies() {
    ...
    if (this.isMikanStrategySetup) { setupStrategies.push('mikan') }
   ...
   */
  resetMikanStrategy() {
    logger.debug('MikanStrategy: reset');
   ...
   */
  setupMikanStrategy() {
    this.resetMikanStrategy();

    const { configManager } = this.crowi;

    const isMikanEnabled = configManager.getConfig('crowi', 'security:passport-mikan:isEnabled');
    ...
    logger.debug('MikanStrategy: setting up..');

    const apiUrl = configManager.getConfig('crowi', 'security:passport-mikan:apiUrl');
    const cookieName = configManager.getConfig('crowi', 'security:passport-mikan:cookieName');
    ...
    logger.debug('MikanStrategy: setup is done');
  }
