...
- Task 4.2: running the *whole* `inline-comment.spec.ts` file against the local dev server is flaky in this environment regardless of this task's change — three full runs failed a different single test (or, at default worker count, a different set of 7) each time, always a popover-visibility or `inline-comment-ready` timeout, never the same test twice. The `visual refresh` block itself passed in every run. Judge this file's health from a scoped run (`-g "visual refresh"`) plus the unstable-identity evidence above, not from a single full-file run.
