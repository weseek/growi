...
- **Adjacent expectations**:
  - 一覧アイテムの編集・削除ボタンは、現在の「常時表示のフッターテキストリンク」から、通常コメント（`CommentControl.tsx`）と同じ「ホバーで現れるアイコンのみ」の操作感に変える。配置場所は既存のヘッダー行（未解決／解決済みバッジ＋解決トグルボタンの隣）に収める——通常コメントには無い要素がヘッダー行にすでに存在するため。なお通常コメント側も、後に発見された不具合（絶対配置が親のパディングを無視していた）の修正で同じヘッダー行flowに揃った
...
**自動テストで担保できない残りの穴**: 「モックアップに忠実か」は自動テストでは判定できない。単体テストが見ているのはクラス名の切り替わりと権限判定だけで、実際の位置・大きさ・余白は `visual-acceptance-checklist.md` の照合に依存している。またホバーで現れる／消えるという挙動そのもの（受け入れ基準5・6）は実ブラウザでしか確認できない。タッチデバイスにはホバーが無いため、そこでの発見しやすさは基準5・6の範囲外のまま残っている。
...
5. The インラインコメント機能 shall ポップオーバーのヘッダー行に状態バッジ（未解決／解決済みのピル）を表示しない（2026-09-11 ユーザー判断による追加。**受け入れ基準3と一部矛盾するように見えるが、3は「解決トグルの仕組み」＝クリックで解決状態を切り替える手段について「バッジをクリックして切り替える」案を採用しないと定めたものであり、バッジ自体の表示・非表示は定めていない。ポップオーバーではバッジを表示せず、解決トグルボタンのみを表示する。一覧アイテムのバッジ表示〈要件1.4〉はこの基準の対象外で、従来どおり表示する**）
6. The インラインコメント機能 shall ポップオーバーから起点コメント・返信の両方を削除できるようにする（2026-09-11 その2、ユーザーが実機フィードバックで判断。**受け入れ基準4「ポップオーバーに削除操作を追加しない」を撤回する。理由: インラインコメントとその返信の仕様差をほぼ無くすため、削除の可否も一覧・ポップオーバー間で揃える。受け入れ基準4は歴史的経緯として残すが、現在の実装が従う基準はこの6である**）
7. The インラインコメント機能 shall ポップオーバーの返信フォームに、起点コメント作成フォーム・編集モードの入力欄と同じメンションピッカー（`@` ボタンから候補を選んで挿入できる操作）を提供する（2026-09-11 その4、ユーザーが実機フィードバックで判断。返信フォームはそれまで独自のプレーンな `<textarea>` だったため、メンション挿入を共有できる実装がなく、モックアップの1行ピル形状〈受け入れ基準1の一部〉を維持できなくなった——編集モードと同じ`MentionAwareCommentInput`を使う箱型の見た目に変わる。受け入れ基準1はポップオーバーの通常表示全体に適用されるが、返信フォームの形状に関する部分はこの7によって上書きされる）

**自動テストで担保できない残りの穴**: 受け入れ基準4は6によって撤回済みで、現在の実装が従うのは6である（番号は付け替えず、4は履歴として残している。他のファイルからの引用を壊さないため）。また、ポップオーバーの外側クリックで閉じる判定は、メンション自動補完のポップアップが `document.body` に portal されるため、実ブラウザでしか壊れているかどうか分からない——単体テストの環境では portal 先も同じツリーに見えるため、この種の閉じてしまう不具合をすり抜ける。
...
**自動テストで担保できない残りの穴**: 受け入れ基準5の「`CommentControl.tsx` 自体は変更しない」という括弧書きは、その後のユーザー判断により撤回されている——編集・削除アイコンの組は共通コンポーネント `CommentEditDeleteButtons` に切り出し、`CommentControl.tsx` はその薄いラッパーになった。さらに、当初は揃っていなかった配置の仕組み（通常コメント側の絶対配置 vs インラインコメント側のヘッダー行flow）も、通常コメント側の絶対配置が親のパディングを無視する不具合だったと判明したことを機に、両者ともヘッダー行の `ms-auto` flow に揃っている。基準の本体（同じアイコン・視覚的パターンを踏襲する）は、共通化によりむしろ強く満たされている。基準3.1（ハードコード色を追加しない）は `client/no-literal-colors.spec.ts` が機械的に検査しているが、基準3.3・3.4（インラインstyleとユーティリティクラスの使い分け）は判断を伴うため、自動検査は無い。
...
**自動テストで担保できない残りの穴**: この要件はそもそも「自動テストでは足りない」という前提の上に立っており、担保しているのは人（または独立したモデル）による目視照合である。ここには、自動化できない弱点が2つ残っている。1つは、タスク単位のレビューは「そのタスクの説明文が触れた指定」しか見ないため、説明文がたまたま触れていない指定は実装されないまま `[x]` になりうること（実際に2件起きた）。もう1つは、この照合が1回きりの判定であり、以降の変更で実装が動いても自動では気づけないこと——実際、タスク4.4のGO判定のあと、実機フィードバックによる13回の追補で見た目は大きく変わっている。チェックリストを再び判定に使うときは、必ず撮り直す。
