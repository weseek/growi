...
拡張要素には「静的テキストを挿入すれば足りるもの（plantuml / callout）」と「専用 UI（モーダル）での設定・編集が要るもの（drawio / lsx）」の2系統がある。後者を扱うための、基盤のコマンドアクションの **「静的挿入（insert）」と「副作用起動（run）」の2種**への一般化は、**実装済み基盤が既に提供**している（`SlashRunAction`）。本スペックはその `run` 契約を消費するのみで、基盤の型・`apply` は変更しない。起動・絞り込み・`/query` 置換・i18n 解決・単一トランザクション挿入・コンテキスト除外（`disallowedIn`）は基盤の挙動を踏襲する。
...
3. When 起動コマンド（drawio / lsx / リンク / テーブルビルダー）が適用される, the 拡張要素コマンド shall `/query` の削除を単一トランザクションで行ったうえで、対応するモーダルを起動する
4. When ブロック要素の起動コマンド（drawio / lsx / テーブルビルダー）が行の途中（`/` の直前に非空白テキストがある、例 `図: /drawio`）で適用される, the 拡張要素コマンド shall モーダルが挿入するブロック要素（` ```drawio ` フェンス / `$lsx(...)` / Markdown テーブル）が独立した行に置かれるよう、起動前に挿入位置を行頭へ正規化する（先行テキストと同一行に連結して描画が壊れることを防ぐ）。インライン要素のリンクはこの正規化の対象外（Req 9.1 参照）
...
4. Where カーソルがリスト項目内またはテーブルセル内にある, the 拡張要素コマンド shall ブロック要素コマンド（drawio / lsx / plantuml / callout / テーブルビルダー）を基盤のコンテキスト除外（`disallowedIn: ['list','table']`）により候補として表示しない。インライン要素（リンク）はこの制限を受けない
