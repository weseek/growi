module.exports = function (crowi, app) {
  ...
  actions.notification.slackAuth = function (req, res) {
    ...
    slack.getOauthAccessToken(code)
      .then(data => {
        ...
        Config.updateNamespaceByArray('notification', {
  ...
  actions.globalNotification.detail = async (req, res) => {
    ...
    if (notificationSettingId) {
      try {
        renderVars.setting = await GlobalNotificationSetting.findOne({
  ...
  actions.globalNotification.update = async (req, res) => {
    const form = req.form.notificationGlobal;
    const setting = await GlobalNotificationSetting.findOne({
      _id: form.id
    });
  ...
  actions.globalNotification.remove = async (req, res) => {
    ...
    try {
      await GlobalNotificationSetting.findOneAndRemove({
        _id: id
      });
      return res.redirect('/admin/notification#global-notification');
    } catch (err) {
  ...
  actions.externalAccount.index = function (req, res) {
    ...
    ExternalAccount.findAllWithPagination({
        page
      })
      .then((result) => {
        const pager = createPager(result.total, result.limit, result.page, result.pages, MAX_PAGE_LIST);
  ...
  actions.externalAccount.remove = function (req, res) {
    ...
    ExternalAccount.findOneAndRemove({
        accountId
      })
      .then((result) => {
        if (result == null) {
  ...
  actions.userGroup.index = function (req, res) {
    ...
    UserGroup.findUserGroupsWithPagination({
        page: page
      })
      .then((result) => {
        const pager = createPager(result.total, result.limit, result.page, result.pages, MAX_PAGE_LIST);
